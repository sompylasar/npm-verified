...
async function asyncCloneAtVersionFromGit({ repoUrl, clonePath, version }) {
  ...
  const cloneTagOrBranch = async (tagOrBranch) => {
    ...
    return await asyncExecuteCommand(
      `git clone --depth 1 --branch "${tagOrBranch}" "${repoUrl}" "${clonePath}"`,
      {
        cwd: clonePath,
      },
    );
  ...
  // Fall back to `master`, we'll compare the version again later.
  const versionTags = ['v' + version, version, 'master'];
  const exceptions = [];
  for (let i = 0; i < versionTags.length; ++i) {
    const versionTagOrBranch = versionTags[i];
    try {
      await cloneTagOrBranch(versionTagOrBranch);
      return {
        tagOrBranch: versionTagOrBranch,
        commit: (await asyncExecuteCommand(
      ...
      exceptions.push(ex);
      debug_asyncCloneAtVersionFromGit(
        'cloneTagOrBranch exception for',
        versionTagOrBranch,
        ex,
      );
    }
...
async function asyncCloneAtVersion({ repoType, repoUrl, clonePath, version }) {
  if (repoType === 'git') {
    return await asyncCloneAtVersionFromGit({ repoUrl, clonePath, version });
  } else {
...
async function main({ tmpdir }) {
  ...
  console.log(
    `Cloned ${chalk.bold(repoUrl)} ` +
      `(${chalk.bold(repoType)} repository) ` +
      `at tag/branch "${chalk.bold(clonedTagOrBranch)}" (commit ${chalk.bold(
        clonedCommit,
      )})...`,
  );
  ...
  console.log(
    `Found ${chalk.bold(packageNameParsed.name)} package.json ` +
      `at ${chalk.bold(clonedPackageRootPathForLog)}...`,
  );

  if (clonedPackageJson.version !== version) {
    process.exitCode = 1;
    console.log(
      `Published package version (${chalk.yellow(version)}) ` +
        `is ${chalk.red.bold('different')} from the version ` +
        `in the source code (${chalk.yellow(clonedPackageJson.version)}).`,
    );
    return;
  }

  console.log(
    `Preparing the package from the cloned repository ` +
      `at ${chalk.bold(clonedPackageRootPathForLog)}...`,
  );
