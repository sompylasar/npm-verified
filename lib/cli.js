...
async function asyncCloneAtVersionFromGit({ repoUrl, clonePath, version }) {
  ...
  // Try two conventions for version tags: `vX.Y.Z` and `X.Y.Z`.
  try {
    await cloneTag('v' + version);
  } catch (ex) {
    await cloneTag(version);
  }
}
...
async function asyncNpmPack({ packageRootPath }) {
  // TODO(@sompylasar): Use `yarn` and `npm` from dependencies, not from environment.
